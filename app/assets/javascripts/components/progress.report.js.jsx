...
var AssignmentScoreBox = React.createClass({
  ...
  render: function() {
    switch (parseInt(this.props.score, 10)) {
      case 0:
        return (
          <div className={'assignment-score-box ' + this.props.className}>
            Incomplete
          </div>
        )
      case 1:
...
var SelectAssignments = React.createClass({
  ...
  updateSelectedList: function(e) {
    if (e.target.checked) {
      // adding to list
      this.setState({
        selectedHomework: this.state.selectedHomework.concat(
          this.props.homework.filter(w => w.id == e.target.value)
        )
      })
    } else {
      // removing from list
      this.setState({
        selectedHomework: this.state.selectedHomework.filter(
              ...
              return (
                <li key={hw.id}>
                  <input
                    ...
                    id={hw.id}
                    value={hw.id}
                    onClick={this.updateSelectedList}
                  />
                  <label htmlFor={hw.id}>
                    <span>{hw.title}</span>
                  </label>
...
var ProgressReport = React.createClass({
        ...
        this.setState(
          {
            studentAssignments: json
              .filter(f => this.state.assignmentIds.includes(f.homework.id))
              .reverse(),
            summary: json
              .filter(f => this.state.assignmentIds.includes(f.homework.id))
              .reduce((acc, item) => {
          ...
          () => {
            ...
            this.setState({
              dateRange: `${firstDate.getMonth() +
                1}/${firstDate.getDate()}/${firstDate.getFullYear()} to ${new Date().getMonth() +
                1}/${new Date().getDate()}/${new Date().getFullYear()}`
            })
          }
        )
  ...
  shouldComponentUpdate: function(nextProps, nextState) {
    const _should =
      ...
      JSON.stringify(this.state.studentAssignments) !==
        JSON.stringify(nextState.studentAssignments)
    return _should
  },
      ...
      <section className="progress-report">
        <header className="report-title">
          <h1>
            {' '}
            <img
              src="/assets/logo-7e1718be455ad960f928d84c77eba86849faf744deb2c6e2d2b63f7fcb0d32b4.png"
              height="64px"
              width="64px"
            />
            Progress Report for {this.props.student.name}
          </h1>
        </header>
        ...
        <section className="report-body">
          {this.state.showForm && (
            <section className="feedback-form">
              <label>
                What is{' '}
                {this.props.student.name.substr(
                  0,
                  this.props.student.name.indexOf(' ')
                )}{' '}
                doing well?
              </label>
              <textarea
              ...
              <textarea
                onChange={this.handleChange}
                name="attendance"
                placeholder={this.state.attendance}
              />
          ...
          {!this.state.showForm && (
            <section className="feedback-report">
              ...
              <label>Attendance:</label>
              <section onChange={this.handleChange} name="attendance">
                {this.state.attendance}
              </section>
              <button className="noprint" onClick={this.goToNextStudent}>
                Next
              </button>
            </section>
          )}
                  ...
                  {Object.keys(this.state.summary).map((sum, i) => {
                    return (
                      <li key={i}>
                        <AssignmentScore
                          score={sum}
                          className={`score_${sum}`}
                        />
                        <span className={`score_${sum}`}>
...
var StudentProgressReport = React.createClass({
  ...
  startProgressReports: function(selectedHomework) {
    const _title = `${new Date().getMonth() +
      1}/${new Date().getDate()}/${new Date().getFullYear()} ${
      this.state.students[this.state.currentStudent.next].name
    }`
      ...
      case 1:
        return (
          <SelectAssignments
            ...
            startProgressReports={this.startProgressReports}
          />
        )
      case 2:
        return (
          <ProgressReport
            startNextProgressReport={this.startNextProgressReport}
            student={this.state.currentStudent.data}
            cohort={this.props.cohort.name}
            assignments={this.state.reportAssignments}
          />
        )
      case 3:
        return <div>done</div>
  ...
  render: function() {
    return <div>{this.getCurrentPage()}</div>
  }
})
